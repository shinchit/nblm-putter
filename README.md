...
指定したフォルダ以下のファイルを再帰的に走査し、まず Google Drive の `nblm-putter/{notebook-id}/` フォルダにアップロードする。その後 NotebookLM の Drive ピッカーで全ファイルを一括追加する（ボタン待ちなし・並列処理）。ターミナルにプログレスバーと処理中ファイル名が表示される。
...
- **Phase 1** でローカルファイルを Google Drive にアップロードし、**Phase 2** で NotebookLM の Drive ピッカーを使って一括追加する。
